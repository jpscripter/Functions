Function Get-MDT_Help
{
  ...
  Process
  {
    If ($blnLog) {Write-MDT_LogMessage -message "-Path : $path" -component $component -type 4}
    ...
    Foreach ($line in $comments){
      ...
      Switch ($true)
      {
        ...
        ($line -like 'region*')
        {
          ...
          {
            If ($blnLog) {Write-MDT_LogMessage -message "-Line number not expected $tabNumber : $line" -component $component -type 4}
          }
        }
  ...
  End
  {
    If ($blnLog) {Write-MDT_LogMessage -message "-Return = $ScriptComments" -component $component -type 4}
